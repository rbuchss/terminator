...
function terminator::python::invoke {
  ...
  for python_provider in "${python_providers[@]}"; do
    terminator::logger::info "Try using python provider: ${python_provider}"

    if python_command="$("terminator::python::invoke::with_${python_provider}" "${major_version}")"; then
      terminator::logger::info "Success ${python_provider} has python command: '${python_command}'"
...
function terminator::python::invoke::with_pyenv {
  ...
  terminator::logger::debug "Attempting to using pyenv python version: ${full_version} -> ${short_version}"

  full_path="$(pyenv root)/shims/python${short_version}"

  if ! [[ -x "${full_path}" ]]; then
    terminator::logger::warning "pyenv installed but no shims match version: '${short_version}' at '${full_path}'"
    return 1
  fi

  terminator::logger::debug "Using pyenv python version: ${full_version} -> ${short_version} at '${full_path}'"
...
function terminator::python::invoke::with_homebrew {
  ...
  if ! [[ -x "${full_path}" ]]; then
    terminator::logger::warning "homebrew installed but no executable matches version: '${prefix_path}' at '${full_path}'"
    return 1
  fi

  terminator::logger::debug "Using homebrew python version: ${prefix_path} at '${full_path}'"
...
function terminator::python::invoke::with_system {
  ...
  if ! full_path="$(command -v "python${major_version}" 2>/dev/null)"; then
    terminator::logger::warning "no system installed python versions match major version: '${major_version}'"
    return 1
  fi
...
function terminator::python::invoke::error {
  ...
  terminator::logger::error "Using python major version ${major_version} not supported"
  return 1
}
